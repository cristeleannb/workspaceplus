...
export const SelectParkingFloorModal: ModalComponentWithOptions<
  ModalComponentProp<ModalStackParamsList, void, 'SelectParkingFloorModal'>
> = ({modal: {closeModal, getParam}}) => {
  const onSelect = (floor: number) => {
    getParam('onSelect')({floor: floor});
  };
  ...
  const selectionsList = getParam('parkingFloorsList');
    ...
    <BaseModal>
      ...
      <View
        ...
        {selectionsList.map((item, index) => (
          <SelectionItem
            key={index}
            label={`Basement ${item.floor}`}
            selected={getParam('selectedFloor').floor === index + 1}
            onSelect={() => onSelect(item.floor || 0)}
          />
        ))}
      </View>
    </BaseModal>
